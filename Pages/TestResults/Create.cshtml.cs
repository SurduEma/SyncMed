...
namespace SyncMed.Pages.TestResults;
...
public class CreateModel : PageModel
{
    ...
    [BindProperty, Required, MaxLength(500), Url]
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        if (User.IsInRole(AppRoles.Doctor))
        {
            ...
            if (patient == null || !patient.Appointments.Any(a => a.DoctorId == DoctorId))
                ModelState.AddModelError(nameof(PatientId), "Doctors can upload results only for their own patients.");
        }
        ...
        var result = await _clinicalRecordService.AddTestResultAsync(PatientId, DoctorId, TestName, DocumentUrl);
        if (!result.Success)
        {
